...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (threadResponse.metadata!["userId"] != userId) {
      throw new Error("User ID does not match queried thread, please sign out and sign in again")
    }
    ...
    while (true) {
      ...
      messages.forEach((datum) => {
        ...
        if (
          (datum.role === "user" && !(datum.content[0]["text"]["value"] as string).includes("<Reminder>")) ||
          (chatAssistantId && datum.role === "assistant" && datum.assistant_id === chatAssistantId)) {
